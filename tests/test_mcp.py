"""Tests for preset MCP install/uninstall (S7, issue #8).
...
Conventions:
- Pure transforms (``build_mcp_entry`` / ``install_into_doc`` /
  ``uninstall_from_doc``) are tested on plain dicts — no FS.
- The high-level ``install_mcp`` / ``uninstall_mcp`` cycle is tested with a
  tmp HOME (autouse ``_isolate_home``) and reads the written JSON back via
  ``JsonBackend`` to assert the on-disk section, not just the in-memory dict.
- Region-awareness is tested both ways (GLOBAL → ``api.z.ai`` / ``ZAI``;
  CHINA → ``open.bigmodel.cn`` / ``ZHIPU``).
"""
...
def test_http_presets_carry_region_aware_url_template(mcp_id):
    ...
    global_url = template["glm_coding_plan_global"]
    china_url = template["glm_coding_plan_china"]
    # Both regions share the same path; only the host swaps.
    assert global_url.startswith("https://api.z.ai/api/mcp/")
    assert china_url.startswith("https://open.bigmodel.cn/api/mcp/")
    assert global_url.rsplit("/", 1)[0] == china_url.rsplit("/", 1)[0].replace(
        "open.bigmodel.cn", "api.z.ai"
    )
    assert global_url.endswith("/mcp")
    assert china_url.endswith("/mcp")
...
def test_opencode_stdio_entry_uses_local_and_command_array():
    """OpenCode stdio: type 'local', command as a single array, 'environment'."""
    entry = build_mcp_entry(Tool.OPENCODE, "zai-mcp-server", _KEY, Region.GLOBAL)
    assert entry == {
        "type": "local",
...
def test_opencode_http_entry_uses_remote_type():
    """OpenCode http: type 'remote' (not 'http')."""
    entry = build_mcp_entry(Tool.OPENCODE, "zread", _KEY, Region.GLOBAL)
    assert entry["type"] == "remote"
    assert entry["url"] == "https://api.z.ai/api/mcp/zread/mcp"
    assert entry["headers"] == {"Authorization": f"Bearer {_KEY}"}
...
def test_crush_matches_claude_code_entry_shape():
    """Crush uses the same entry shape as Claude Code (mcp section, http type)."""
...
# --------------------------------------------------------------------------- #
# Region awareness — GLOBAL vs CHINA.
# --------------------------------------------------------------------------- #
...
def test_region_global_uses_api_z_ai_and_zai_mode():
    """GLOBAL region (default): api.z.ai host + Z_AI_MODE=ZAI."""
...
# --------------------------------------------------------------------------- #
# Auth handling — key omitted, Bearer header, stdio env key.
# --------------------------------------------------------------------------- #
...
def test_install_without_key_omits_auth():
    ...
    assert "Z_AI_API_KEY" not in stdio["env"]
    assert stdio["env"] == {"Z_AI_MODE": "ZAI"}
...
def test_install_into_doc_creates_section_when_absent():
    """Installing into a None doc creates the section with just this entry."""
    doc = install_into_doc(None, Tool.CLAUDE_CODE, "zread", _KEY, Region.GLOBAL)
    assert doc == {
        "mcpServers": {
...
def test_install_into_doc_preserves_foreign_keys_and_entries():
    """Foreign top-level keys AND foreign MCP entries survive an install."""
    prior = {
        "hasCompletedOnboarding": True,  # foreign top-level key (Claude Code)
        "mcpServers": {
            "user-custom": {"type": "stdio", "command": "echo"},  # foreign entry
        },
    }
    ...
    # Foreign MCP entry preserved.
    assert doc["mcpServers"]["user-custom"] == prior["mcpServers"]["user-custom"]
    # New entry present.
...
def test_install_into_doc_uses_correct_section_per_tool():
    """OpenCode/Crush write to 'mcp'; Claude Code/Factory Droid to 'mcpServers'."""
...
def test_uninstall_from_doc_removes_only_its_id():
    """uninstall removes ONLY the named id; siblings + foreign keys survive."""
    doc = {
    ...
    # Siblings + foreign survive.
    assert "web-search-prime" in out["mcpServers"]
    assert "user-custom" in out["mcpServers"]
    assert out["hasCompletedOnboarding"] is True
    # Input not mutated.
    assert "zread" in doc["mcpServers"]


def test_uninstall_drops_empty_section():
    """Removing the last entry drops the section entirely (no stray empty dict)."""
...
def test_uninstall_idempotent_on_absent_id():
    """Removing an absent id is a no-op (no write, returns a copy of input)."""
    doc = {"mcpServers": {"zread": {"type": "http"}}}
    out = uninstall_from_doc(doc, Tool.CLAUDE_CODE, "web-reader")
    assert out == doc
    # None doc -> empty dict, not a crash.
...
# --------------------------------------------------------------------------- #
# tool_config_path — pure path resolution per tool.
# --------------------------------------------------------------------------- #
...
def test_install_mcp_preserves_existing_foreign_content(_isolate_home):
    """install_mcp deep-merges; existing top-level keys + entries are kept."""
    path = tool_config_path(Tool.CLAUDE_CODE, _isolate_home)
...
def test_install_mcp_is_idempotent(_isolate_home):
    """Re-installing the same id with the same value writes nothing."""
    install_mcp(Tool.CLAUDE_CODE, "zread", _KEY, Region.GLOBAL, home=_isolate_home)
    changed = install_mcp(
        Tool.CLAUDE_CODE, "zread", _KEY, Region.GLOBAL, home=_isolate_home
    )
...
def test_install_mcp_accepts_string_tool(_isolate_home):
    ...
    changed = install_mcp(
        "claude-code", "zread", _KEY, Region.GLOBAL, home=_isolate_home
    )
    assert changed is True
    assert is_installed(
        JsonBackend.read(tool_config_path(Tool.CLAUDE_CODE, _isolate_home)),
        Tool.CLAUDE_CODE,
        "zread",
    )


def test_uninstall_mcp_removes_entry_from_disk(_isolate_home):
    """uninstall_mcp removes the id and leaves siblings on disk."""
    install_mcp(Tool.CLAUDE_CODE, "zread", _KEY, Region.GLOBAL, home=_isolate_home)
    install_mcp(
        Tool.CLAUDE_CODE, "web-reader", _KEY, Region.GLOBAL, home=_isolate_home
    )
    removed = uninstall_mcp(Tool.CLAUDE_CODE, "zread", home=_isolate_home)
...
def test_uninstall_mcp_idempotent_when_absent(_isolate_home):
    """uninstall of an absent id is a no-op (no write, returns False)."""
    removed = uninstall_mcp(Tool.CLAUDE_CODE, "zread", home=_isolate_home)
    assert removed is False
    # No file written.
    assert not tool_config_path(Tool.CLAUDE_CODE, _isolate_home).exists()
...
def test_install_mcp_uses_injected_io_seams(_isolate_home):
    ...
    # OpenCode section is 'mcp', entry type is 'remote'.
    assert store[path]["mcp"]["zread"]["type"] == "remote"


# --------------------------------------------------------------------------- #
# JSON on disk is well-formed (atomic + trailing newline, matches backend).
# --------------------------------------------------------------------------- #
...
def test_written_config_is_valid_json_with_trailing_newline(_isolate_home):
    ...
    # Round-trips through plain json (not just JsonBackend).
    json.loads(text)


# --------------------------------------------------------------------------- #
# doctor — the MCP probe reports installed presets (READ-ONLY, never fails).
# --------------------------------------------------------------------------- #


def test_doctor_mcp_probe_reports_none_installed_by_default(_isolate_home, capsys):
    """With no MCP installed, doctor reports 'none installed (opt-in)' — a PASS."""
    from zai_python_helper.doctor import ProbeResult, run_doctor

    _write_minimal_zai_settings(_isolate_home)
    rc = run_doctor(
        Paths.from_home(_isolate_home),
        color=False,
        environ={},
        http_get=lambda url, headers, body: ProbeResult(status=None, error="offline"),
    )
    captured = capsys.readouterr().out
    assert "preset MCP servers" in captured
    assert "none installed" in captured
    # MCP is PASS-only; the offline probe is a WARN, so doctor stays at exit 0.
    assert rc == 0


def test_doctor_mcp_probe_passes_when_mcp_installed(_isolate_home, capsys):
    """An installed preset MCP surfaces as a PASS detail naming it."""
    from zai_python_helper.doctor import run_doctor

    _write_minimal_zai_settings(_isolate_home)
    install_mcp(Tool.CLAUDE_CODE, "zread", _KEY, Region.GLOBAL, home=_isolate_home)
    # Stub the HTTP probe so the run is offline + deterministic; the MCP probe
    # itself does no network, but we isolate the whole run from the real net.
    from zai_python_helper.doctor import ProbeResult
    ...
    captured = capsys.readouterr().out
    assert "preset MCP servers" in captured
    assert "zread@claude-code" in captured
    # An installed MCP must not fail doctor.
    assert rc == 0
