...
*enum — [source](https://github.com/axisrow/zai_python_helper/blob/main/src/zai_python_helper/mcp.py#L82)*

A coding assistant whose MCP config we can patch.

`str` enum so CLI values (`--tool claude-code`) round-trip through
`Tool` directly. The value is the canonical tool id used in the
upstream managers.
